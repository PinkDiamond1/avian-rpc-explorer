...
const debugLog = debug("avnexp:router");
...
const avianjs = require('bitcoinjs-lib');
...
router.get("/", asyncHandler(async (req, res, next) => {
	try {
		...
		promises.push(utils.timePromise("homepage.getSmartFeeEstimates", async () => {
			...
			smartFeeEstimates[feeConfTargets[0]] = 0.015;
			smartFeeEstimates[feeConfTargets[1]] = 0.012;
			smartFeeEstimates[feeConfTargets[2]] = 0.010;
...
router.get("/block-stats", asyncHandler(async (req, res, next) => {
	if (semver.lt(global.avnNodeSemver, rpcApi.minRpcVersions.getblockstats)) {
...
router.get("/block-height/:blockHeight", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Avian Block #${blockHeight.toLocaleString()}`;
			res.locals.metaDesc = "";
		}
...
router.get("/block/:blockHash", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Avian Block ${utils.ellipsizeMiddle(res.locals.result.getblock.hash, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/tx/:transactionId", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialTransactions && global.specialTransactions[txid]) {
			...
			res.locals.metaTitle = `Avian Transaction ${utils.ellipsizeMiddle(txid, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/address/:address", asyncHandler(async (req, res, next) => {
	try {
		...
		res.locals.metaTitle = `Avian Address ${address}`;
		...
		let b58prefix = (global.activeBlockchain == "main" ? /^[R].*$/ : /^[2mn].*$/);
		if (address.match(b58prefix)) {
			try {
				res.locals.addressObj = avianjs.address.fromBase58Check(address);
		...
		if (addressEncoding == "unknown") {
			try {
				res.locals.addressObj = avianjs.address.fromBech32(address);
...
router.get("/rpc-terminal", function(req, res, next) {
	if (!config.demoSite && !req.authenticated) {
		res.send("RPC Terminal / Browser require authentication. Set an authentication password via the 'AVNEXP_BASIC_AUTH_PASSWORD' environment variable (see .env-sample file for more info).");
...
router.post("/terminal", function(req, res, next) {
	...
	if (cmd == "parsescript") {
		const nbs = require('node-avian-script');
...
router.get("/quotes", function(req, res, next) {
	res.locals.avnQuotes = avnQuotes.items;
...
router.get("/holidays", function(req, res, next) {
	res.locals.avnHolidays = global.avnHolidays;
...
router.get("/avian-whitepaper", function(req, res, next) {
	res.render("avian-whitepaper");
