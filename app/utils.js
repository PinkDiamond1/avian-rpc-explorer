...
const debugLog = debug("avnexp:utils");
const debugErrorLog = debug("avnexp:error");
const debugErrorVerboseLog = debug("avnexp:errorVerbose");
...
const bip32TestnetNetwork = {
	messagePrefix: '\x18Avian Signed Message:\n',
	bech32: 'tb',
	bip32: {
		public: 0x043587cf,
		private: 0x04358394,
...
function bip32Addresses(extPubkey, addressType, account, limit=10, offset=0) {
	...
	for (let i = offset; i < (offset + limit); i++) {
		...
		if (addressType == "p2pkh") {
			addresses.push(avianjs.payments.p2pkh({ pubkey: publicKey, network: network }).address);

		} else if (addressType == "p2sh(p2wpkh)") {
			addresses.push(avianjs.payments.p2sh({ redeem: avianjs.payments.p2wpkh({ pubkey: publicKey, network: network })}).address);

		} else if (addressType == "p2wpkh") {
			addresses.push(avianjs.payments.p2wpkh({ pubkey: publicKey, network: network }).address);
